...
describe('MoodGrid', () => {
  ...
  it('labels RESET as "Relax" without renaming the backend key', async () => {
    const onSelect = vi.fn()
    render(<MoodGrid moods={profiles} onSelect={onSelect} />)

    expect(screen.queryByText('Reset')).not.toBeInTheDocument()
    await userEvent.click(screen.getByText('Relax'))
  ...
  it('orders tiles by tileOrder, not array order', () => {
    // RESET (tileOrder 4), CHILL (1), PETS (6) — fed in deliberately out of order.
    ...
    expect(names[1]).toContain('Relax')
